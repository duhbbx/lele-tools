...
Base64EncodeDecode::Base64EncodeDecode() : QWidget(nullptr), DynamicObjectBase() {
    ...
    // 设置默认示例
    QString sampleText = tr("Hello, 乐乐的工具箱！\n这是一个Base64编码解码工具。");
...
void Base64EncodeDecode::setupToolbar() {
    ...
    // 创建按钮
    encodeBtn = new QPushButton(tr("🔒 编码"));
    encodeBtn->setToolTip(tr("将文本编码为Base64格式"));

    decodeBtn = new QPushButton(tr("🔓 解码"));
    decodeBtn->setToolTip(tr("将Base64解码为原始文本"));
...
void Base64EncodeDecode::setupInputOutput() {
    ...
    inputLabel = new QLabel(tr("📝 输入文本"));
    inputLabel->setStyleSheet("font-weight: bold; font-size: 12pt; color: #333;");

    inputTextEdit = new QTextEdit();
    inputTextEdit->setPlaceholderText(tr("请输入要编码或解码的文本..."));
...
void Base64EncodeDecode::onEncodeClicked() {
    ...
    if (inputText.isEmpty()) {
        updateStatus(tr("输入为空，请输入要编码的文本"), true);
        return;
    }
    ...
    try {
        ...
        outputLabel->setText(tr("📄 Base64编码结果"));
        updateStatus(tr("编码完成，输入%1字符，输出%2字符").arg(inputText.length()).arg(base64Result.length()), false);
    } catch (const std::exception& e) {
        updateStatus(tr("编码失败: %1").arg(e.what()), true);
    }
}
...
void Base64EncodeDecode::onDecodeClicked() {
    ...
    if (inputText.isEmpty()) {
        updateStatus(tr("输入为空，请输入要解码的Base64文本"), true);
        return;
    }
    ...
    try {
        ...
        if (decodedData.isEmpty() && !inputText.isEmpty()) {
            updateStatus(tr("解码失败，输入的Base64格式可能不正确"), true);
            return;
        }

        QString result = QString::fromUtf8(decodedData);
        outputTextEdit->setPlainText(result);
        outputLabel->setText(tr("📄 Base64解码结果"));
        updateStatus(tr("解码完成，输入%1字符，输出%2字符").arg(inputText.length()).arg(result.length()), false);
    } catch (const std::exception& e) {
        updateStatus(tr("解码失败: %1").arg(e.what()), true);
    }
}
...
void Base64EncodeDecode::onInputTextChanged() {
    ...
    if (text.isEmpty()) {
        ...
        updateStatus(tr("输入文本长度: %1 字符").arg(text.length()), false);
    }
}
