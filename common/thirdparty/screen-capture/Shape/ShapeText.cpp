#include <qpainter.h>
#include <QTransform>
#include <QTextDocument>
#include <numbers>
#include <QTextCharFormat>
#include <QTextOption>
#include <QTimer>

#include "ShapeText.h"
#include "ShapeTextInput.h"
#include "Win/WinBase.h"
#include "Win/WinPin.h"
#include "App/App.h"
#include "App/Logger.h"
#include "Tool/ToolSub.h"
#include "Win/Canvas.h"
#include "App/Util.h"


ShapeText::ShapeText(QObject* parent) : ShapeBase(parent)
{
    LOG_DEBUG("ShapeText", "=== ShapeText构造函数开始 ===");
    
    auto win = (WinBase*)parent;
    if (!win) {
        LOG_ERROR("ShapeText", "父窗口为空");
        return;
    }
    LOG_DEBUG("ShapeText", QString("父窗口指针有效: 0x%1").arg(reinterpret_cast<quintptr>(win), 0, 16));
    
    if (!win->toolSub) {
        LOG_ERROR("ShapeText", "toolSub为空，无法获取工具设置");
        return;
    }
    LOG_DEBUG("ShapeText", QString("toolSub指针有效: 0x%1").arg(reinterpret_cast<quintptr>(win->toolSub), 0, 16));
    
    try {
        LOG_DEBUG("ShapeText", "获取颜色设置");
        color = win->toolSub->getColor();
        LOG_DEBUG("ShapeText", QString("颜色: RGB(%1,%2,%3)")
                  .arg(color.red()).arg(color.green()).arg(color.blue()));
        
        LOG_DEBUG("ShapeText", "获取字体大小");
        fontSize = win->toolSub->getStrokeWidth();
        LOG_DEBUG("ShapeText", QString("字体大小: %1").arg(fontSize));
        
        LOG_DEBUG("ShapeText", "获取粗体设置");
        bold = win->toolSub->getSelectState("bold");
        LOG_DEBUG("ShapeText", QString("粗体: %1").arg(bold ? "是" : "否"));
        
        LOG_DEBUG("ShapeText", "获取斜体设置");
        italic = win->toolSub->getSelectState("italic");
        LOG_DEBUG("ShapeText", QString("斜体: %1").arg(italic ? "是" : "否"));
        
    } catch (const std::exception& e) {
        LOG_ERROR("ShapeText", QString("获取工具设置时发生异常: %1").arg(e.what()));
    } catch (...) {
        LOG_ERROR("ShapeText", "获取工具设置时发生未知异常");
    }
    
    LOG_DEBUG("ShapeText", "=== ShapeText构造函数完成 ===");
}

ShapeText::~ShapeText()
{
}

void ShapeText::paint(QPainter* painter)
{
    if (state != ShapeState::ready) return;
    auto f = Util::getTextFont(fontSize);
    f->setWeight(bold ? QFont::Bold : QFont::Normal);
    f->setItalic(italic);
	painter->setFont(*f);
    painter->setPen(color);
	painter->drawText(containerRect.adjusted(7,-1,0,0), Qt::AlignLeft | Qt::AlignVCenter, text);
}
void ShapeText::paintDragger(QPainter* painter)
{
    QPen pen;
    pen.setColor(color);
    pen.setStyle(Qt::CustomDashLine);
    pen.setDashPattern({ 3,3 });
    pen.setWidth(1);
    painter->setPen(pen);
    painter->setBrush(Qt::NoBrush);
    painter->drawRect(containerRect.adjusted(1,1,-2,-2));
}
bool ShapeText::mouseMove(QMouseEvent* event)
{
    if (state != ShapeState::ready) return false;
    auto ppp = event->pos();
    auto win = (WinBase*)parent();
    if (containerRect.contains(ppp)) {
        win->setCursor(Qt::IBeamCursor);
        hoverDraggerIndex = 8;
        win->canvas->setHoverShape(this);
        return true;
    }
    else {
		hoverDraggerIndex = -1;
        return false;
    }
}
bool ShapeText::mousePress(QMouseEvent* event)
{
    auto win = (WinBase*)parent();
    auto ppp = event->pos();
    if (containerRect.contains(ppp)) {
        state = ShapeState::moving;
        win->canvas->shapeCur = nullptr;
        win->canvas->shapeHover = nullptr;
        win->canvas->removeShapeFromBoard(this);
        container = new ShapeTextContainer(this,win);
        container->move(containerRect.topLeft());
        container->show();
        container->shapeTextInput->setText(text);
        win->canvas->shapeCur = nullptr;
        return true;
    }
    if(state == ShapeState::temp) {
        container = new ShapeTextContainer(this,win);
        container->move(ppp + QPoint(-10, -10));
        container->show();
        container->shapeTextInput->setText("");  //触发一次adjustSize
        return true;
    }
    return false;
}
bool ShapeText::mouseRelease(QMouseEvent* event)
{
    return true;
}
