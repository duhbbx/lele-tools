...
void WinFull::mouseReleaseEvent(QMouseEvent* event) {
    if (state <= State::tool) {
        ...
        const auto customCap = App::getCustomCap();
        ...
        if (customCap == 0) {
            ...
            state = State::tool;
            LOG_DEBUG(MODULE_WIN, QString("设置state为tool: %1").arg(static_cast<int>(state)));

            if (!toolMain) {
                LOG_INFO(MODULE_WIN, "############### 创建新的ToolMain工具栏");
                toolMain = new ToolMain(this);
                LOG_DEBUG(MODULE_WIN, QString("################ ToolMain创建完成，指针地址: 0x%1").arg(reinterpret_cast<quintptr>(toolMain), 0, 16));
            } else {
...
void WinFull::saveToFile() {
    ...
    if (const auto img = getTargetImg(); Util::saveToFile(img)) {
        close(); // 关闭窗口，但不退出应用程序
    }
}
