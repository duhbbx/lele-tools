...
Btn::Btn(const QString& name, const QChar& icon, const bool isEnable, QWidget* parent) : BtnBase(name, icon, parent),
    isEnable { isEnable } {
...
void Btn::setEnable(const bool flag) {
    if (flag == isEnable)
        return;
    isEnable = flag;
    if (flag) {
...
void Btn::paintEvent(QPaintEvent* event) {
    ...
    p.setPen(Qt::NoPen);
    const auto font = Util::getIconFont(15);
    p.setFont(*font);
    if (isEnable) {
...
void Btn::mousePressEvent(QMouseEvent* event) {
    if (event->buttons() & Qt::LeftButton) {
        if (auto* toolBase = dynamic_cast<ToolBase*>(parent())) {
